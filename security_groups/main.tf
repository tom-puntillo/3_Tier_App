...
resource "aws_security_group" "allow_mysql" {
  name        = "db_security_group"            # Set the name of the security group to "web_security_group"
  description = "Allows MySQL inbound traffic" # Provide a description for the security group
  vpc_id      = var.vpc_id                     # Set the VPC ID for the security group based on the input variable 'vpc_id'

  # Define inbound traffic rules for HTTP
  ingress {
    description     = "MySQL from Logic Tier"                          # Set the description for the ingress rule
    to_port         = 3306                                             # Allow traffic to port 80
    from_port       = 3306                                             # Allow traffic from port 80
    protocol        = "tcp"                                            # Allow TCP protocol
    security_groups = [aws_security_group.allow_http_and_tls_logic.id] # Allow traffic from any IPv4 address
  }
  ...
  # Set tags for the security group
  tags = {
    Name = "allow_mysql_traffic_from_logic_tier" # Set the tag 'Name' to "allow_traffic_from_internet"
  }
}